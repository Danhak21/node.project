    ...
    if(req.file){
        ...
        .then(r=>{
          if(r.length){
            ...
            return Model.update("users", r.id, {photo:req.file.filename})
          }
        })
        .then(r=>{
          res.send({success:"OK"})
      ...
      res.send({error:"Can't upload file"})
    }
  ...
  if(req.file){
      ...
      .then(r=>{
        if(r.length){
          r = r[0]
          console.log(req.file)
          return Model.add("posts", {id:uniqid(), text:req.body.text, user:r.id, photo:req.file.filename})
        }
      })
      .then(r=>{
        res.send({success:"OK"})
    ...
    Model.find("users", elm=>elm.token == req.body.token)
         .then(r=>{
           if(r){
             ...
             res.send({error:"TOKEN IS NOT MATCH!"})
           }
         })
         .then(r=>{
           res.send({status:"ok"})
  ...
  Model.find("users", elm => elm.token == req.body.token)
       ...
       .then(async(r)=>{
         if(r.length){
          ...
          return Model.add("likes",{id:uniqid(), user_id:user, user: acc, post_id:req.body.post_id})           
        ...
        .then(async(r)=>{
          for(let i = 0; i < r.length; i++){
            console.log(r[i])
              ...
              r[i].account = account.length? {name:account[0].name, surname:account[0].surname} : null
...
app.post("/addPhoto", upload.single("avatar"), (req,res)=>{
    Model.find("users", elm => elm.token == req.body.token)
         .then(r=>{
           if(r.length){
             r = r[0]
             return  Model.add("photos", {id:uniqid(), privacy:0, photo:req.file.filename, text:req.body.text, user:r.id})
...
app.post("/getMyPhotos", (req,res)=>{
    Model.find("users", elm => elm.token == req.body.token)
         .then(r=>{
           if(r.length){
             r = r[0]
            return  Model.find("photos", elm=>elm.user == r.id)
...
app.post("/changePhotoStatus", (req, res)=>{
  Model.find("users", elm => elm.token == req.body.token)
       ...
       .then(r=>{
         if(r){
            return Model.update("photos", r[0].id, {privacy:req.body.privacy})
         }
       })
       .then(r=>{
         res.send({status:'ok'})
...
app.post("/getUserById", (req, res)=>{
   Model.find("users", elm=>elm.id == req.body.id)
        .then( async (r)=>{
          ...
          if(r){
            // r = r[0]
            let obj = {name:r[0].name, id:r[0].id, surname:r[0].surname}
            obj.posts = await Model.find("posts", elm => elm.user == r.id)
            obj.album = await Model.find("photos", elm => elm.user == r.id)
            return res.send(obj)
          }
        })
})
